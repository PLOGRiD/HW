...
int main() {
    ...
    while(true){
        ...
        if(dummy==1){
            ...
            std::thread eventThread(event_for_AI_Thread, std::ref(cam), std::ref(spectro), std::ref(led), std::ref(servo));
            eventThread.join();
        }
        else if(dummy==2){
            // 서버 연동 AI 테스트
            ...
            std::thread eventThread(event_for_test_Thread,  std::ref(cam), std::ref(spectro), std::ref(led),std::ref(servo));
            std::thread httpThread(http_transmission_thread);
            std::thread gpsThread(gps_thread);
