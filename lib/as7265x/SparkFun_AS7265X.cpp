// SparkFun_AS7265x_Arduino_Library (https://github.com/sparkfun/SparkFun_AS7265x_Arduino_Library/tree/main) -> 라즈베리파이 wiringPi 포팅
...
bool AS7265X::begin()
{
  _i2c_fd = wiringPiI2CSetup(AS7265X_ADDR); // 파일 디스크립터 반환
...
bool AS7265X::isConnected()
{
  ...
  {
    int result = wiringPiI2CReadReg8(_i2c_fd, AS7265X_STATUS_REG);
    if (result != -1){
        return true;
    }
    delay(10);
...
uint8_t AS7265X::virtualReadRegister(uint8_t virtualAddr)
{
  ...
  if(!writeRegister(AS7265X_WRITE_REG, virtualAddr)){
    return 0;
  }
...
void AS7265X::virtualWriteRegister(uint8_t virtualAddr, uint8_t dataToWrite)
{
  ...
  if (!writeRegister(AS7265X_WRITE_REG, (virtualAddr | 1 << 7))){
    return;
  }
  ...
  if (!writeRegister(AS7265X_WRITE_REG, dataToWrite)){
    return;
  }
}
...
{
  int incoming = wiringPiI2CReadReg8(_i2c_fd, addr);
...
bool AS7265X::writeRegister(uint8_t addr, uint8_t val)
{
  int result = wiringPiI2CWriteReg8(_i2c_fd, addr, val);
